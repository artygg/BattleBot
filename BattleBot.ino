...
void turnLeft(int deg) {
  for (int totalPulses = 0; totalPulses < deg / 2.6;) {
    left();
...
void forward(int distance) {
  static unsigned int totalPulses;
  if (totalPulses < (distance - distanceLimit) / pulseDistance) {
    if(robotState == 0) {
      totalPulses = 0;
      drive(a1Motor1, a2Motor1, 250);
